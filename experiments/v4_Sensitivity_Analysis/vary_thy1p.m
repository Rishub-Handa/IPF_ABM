...
[D, idd, id] = unique(data_mat(:,t1p_idx)); 
...
figure(1)
hold on 
errorbar(data_agg(:, t1p_idx), data_agg(:, AS_idx), data_agg_std(:, AS_idx), '-bo', 'LineWidth', 2); 
errorbar(data_agg(:, t1p_idx), data_agg(:, med_idx), data_agg_std(:, med_idx), '-o', 'LineWidth', 2); 
errorbar(data_agg(:, t1p_idx), data_agg(:, stiff_idx), data_agg_std(:, stiff_idx), '-ro', 'LineWidth', 2); 
xlabel("Thy1p Count"); 
ylabel("% Patches"); 
title("% Patches with Collagen vs. Thy1p Count w Error Bars"); 
legend("% Alveolar Space w Collagen", "% Medium Patches", "% Stiff Patches"); 
hold off 

figure(2)
hold on 
errorbar(data_agg(:, t1p_idx), data_agg(:, AS_idx), data_agg_conf(:, AS_idx), '-bo', 'LineWidth', 2); 
errorbar(data_agg(:, t1p_idx), data_agg(:, med_idx), data_agg_conf(:, med_idx), '-o', 'LineWidth', 2); 
errorbar(data_agg(:, t1p_idx), data_agg(:, stiff_idx), data_agg_conf(:, stiff_idx), '-ro', 'LineWidth', 2); 
xlabel("Thy1p Count"); 
ylabel("% Patches"); 
title("% Patches with Collagen vs. Thy1p Count w Conf Interval"); 
legend("% Alveolar Space w Collagen", "% Medium Patches", "% Stiff Patches"); 
hold off 
