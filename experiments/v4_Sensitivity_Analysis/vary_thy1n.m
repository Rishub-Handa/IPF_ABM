...
hold on 
plot(data_agg(:, t1n_idx), data_agg(:, AS_idx), '-bo', 'LineWidth', 2); 
plot(data_agg(:, t1n_idx), data_agg(:, med_idx), '-o', 'LineWidth', 2); 
plot(data_agg(:, t1n_idx), data_agg(:, stiff_idx), '-ro', 'LineWidth', 2); 
xlabel("Thy1n Count"); 
ylabel("% Patches"); 
title("% Patches with Collagen vs. Thy1n Count"); 
